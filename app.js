...
const TEXTS = {
  en: {
    back: '⟵ Back',
    welcomeTitle: 'Arivoli ',
    welcomeSubtitle: 'MCQs for Young Minds — explore Science and level up as you learn!',
    letsBegin: "Let's Begin",
    featureLevelsTitle: 'Three Levels',
    featureLevelsDesc: 'Beginner  Intermediate  Advanced',
    featureBadgesTitle: 'Science Fields',
    featureBadgesDesc:'Physics  Chemistry  Biology',
    featureBilingualTitle: 'Bilingual',
    featureBilingualDesc:'Available in English and Tamil',
  ...
  ta: {
    back: '⟵ பின்',
    welcomeTitle: 'அறிவொளி ',
    welcomeSubtitle: 'அறிவியலை சுவாரஸ்யமாக கற்றுக்கொள்ள வினாடி வினா!',
    letsBegin: 'தொடங்குவோம்',
    featureLevelsTitle: 'மூன்று நிலைகள்',
    featureLevelsDesc: 'தொடக்க  நடுநிலை  மேம்பட்ட',
    featureBadgesTitle: 'அறிவியல் துறைகள்',
   featureBadgesDesc:'இயற்பியல்  வேதியியல்  உயிரியல்',
    featureBilingualTitle: 'இருமொழி',
   featureBilingualDesc:'தமிழ் மற்றும் ஆங்கிலத்தில் கிடைக்கும்',
...
function bindUI(){
  elements.langButtons.forEach(b => b.addEventListener('click', () => {
    ...
    persistProgress();
    applyLanguage();
    if(!elements.welcomeSection.hidden){
      showWelcomePage();
    } else if(!elements.avatarSection.hidden){
      showAvatarSelection();
  ...
  elements.backBtn.addEventListener('click', () => {
    // Sequential back: question -> subject -> grade -> level -> avatar -> welcome
    if(!elements.questionCard.hidden){
      // From question back to subject selection
    ...
    if(elements.avatarSection && !elements.avatarSection.hidden){
      // From avatar back to welcome
      showWelcomePage();
      elements.backBtn.hidden = true; // on welcome screen, hide back
      return;
    }
  });
}
...
async function init(){
  ...
  if(elements.welcomeSection){
    showWelcomePage();
  } else if(elements.avatarSection){
    showAvatarSelection();
  } else {
...
function showGradeSelection(){
  ...
  elements.welcomeSection && (elements.welcomeSection.hidden = true);
  elements.avatarSection && (elements.avatarSection.hidden = true);
  elements.landing.hidden = true;
  if(elements.gradeSection) elements.gradeSection.hidden = false;
...
function showWelcomePage(){
  ...
  elements.welcomeSection.hidden = false;
...
function showLevelSelection(){
  ...
  elements.landing.hidden = false;
  if(elements.welcomeSection) elements.welcomeSection.hidden = true;
  if(elements.avatarSection) elements.avatarSection.hidden = true;
  if(elements.gradeSection) elements.gradeSection.hidden = true;
  if(elements.subjectSection) elements.subjectSection.hidden = true;
  if(elements.questionCard) elements.questionCard.hidden = true;
...
function showAvatarSelection(){
  ...
  elements.welcomeSection.hidden = true;
  elements.landing.hidden = true;
  elements.gradeSection && (elements.gradeSection.hidden = true);
  elements.subjectSection && (elements.subjectSection.hidden = true);
  elements.questionCard.hidden = true;
...
function applyLanguage(){
  ...
  if(elements.welcomeSection){
    ...
    const featureLevelsTitle = document.getElementById('feature-levels-title');
    const featureLevelsDesc = document.getElementById('feature-levels-desc');
    const featureBadgesTitle = document.getElementById('feature-badges-title');
    const featureBadgesDesc = document.getElementById('feature-badges-desc');
    const featureBilingualTitle = document.getElementById('feature-bilingual-title');
    const featureBilingualDesc = document.getElementById('feature-bilingual-desc');
    
    if(welcomeTitle) welcomeTitle.textContent = TEXTS[state.language].welcomeTitle;
    if(welcomeSub) welcomeSub.textContent = TEXTS[state.language].welcomeSubtitle;
    if(beginBtn) beginBtn.textContent = TEXTS[state.language].letsBegin + ' 🚀';
    if(featureLevelsTitle) featureLevelsTitle.textContent = TEXTS[state.language].featureLevelsTitle;
    if(featureLevelsDesc) featureLevelsDesc.textContent = TEXTS[state.language].featureLevelsDesc;
    if(featureBadgesTitle) featureBadgesTitle.textContent = TEXTS[state.language].featureBadgesTitle;
    if(featureBadgesDesc) featureBadgesDesc.textContent = TEXTS[state.language].featureBadgesDesc;
    if(featureBilingualTitle) featureBilingualTitle.textContent = TEXTS[state.language].featureBilingualTitle;
    if(featureBilingualDesc) featureBilingualDesc.textContent = TEXTS[state.language].featureBilingualDesc;
  }
